...
exports.create_follower = async (req, res, next) => {
    if(req.userData._id === req.body.following){
        ...
        if (user) {
            ...
            }).select(" _id firstName lastName profileImage").exec().catch(err => response(res, 404, false, "following does not exist", err))
            if (following) {
                ...
                response(res, 200, true, "successful operation", follow)
            }
        }
    }
    ...
    Follower
        ...
        .then(data => {
            data.n !== 0 ?
                ...
                response(res, 404, true, "follow does not exist ")
        })
        .catch(err => {
            response(res, 500, false, "error", err)
        })
